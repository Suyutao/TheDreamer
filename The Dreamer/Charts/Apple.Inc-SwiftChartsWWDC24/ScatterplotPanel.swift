...
private struct Scatterplot: View {
    ...
    var body: some View {
        ...
        Chart {
            ...
            LinePlot(x: "x", y: "y") { @Sendable x in
                regression(x)
            }
            .foregroundStyle(colorScheme == .dark ? .white : .black)
            .lineStyle(.init(lineWidth: colorScheme == .dark ? 1.5 : 1))

            AreaPlot(x: "x", yStart: "Confidence interval low", yEnd: "Confidence interval high") {
                @Sendable x in
                let interval = confidenceInterval(x)
                return (yStart: interval.low, yEnd: interval.high)
            }
            .foregroundStyle(colorScheme == .dark ? .white : .black)
            .opacity(0.3)
        }
